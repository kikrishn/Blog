...
The crucial point to note here that the above equations holds for any bilinear map $V \times W \to F$. Here is the reformulation to avoid repetition of the phrase "valid for all bilinear maps $V \times W \to F$": Let $\mathcal{B_X}$  denote the set of bilinear maps $V \times W \to X$, and for each $v \in V$ and $w \in W$ let $[v,w]$ denote the function
